...
# List of datasets
db_list = ["sift", "gist", "tripclick", "laion", "ytb_video", "ytb_audio","yfcc"]
...
def create_plot(metric="QPS"):
    ...
    fig, axs = plt.subplots(
        ...
        figsize=(len(db_list) * 3.5, len(query_types) * 2.8), 
        dpi=150
    )
    ...
    for i, query_type in enumerate(query_types):
        for j, db in enumerate(db_list):
            ...
            if j == 0:
                ...
                if metric == "Cmps":
                    print_metric="DC"
                ax.set_ylabel(f"{query_type_beautify(query_type)}\n{print_metric}", fontsize=22, labelpad=0)
            
            if i == len(query_types) - 1:
                ax.set_xlabel(f"Recall@10", fontsize=22)
